...
//网络层采用UDP协议
unsigned char UDP[65535];//UDP的最大长度为65535字节，因为IP数据报的最大长度为2^16，需要减去IP首部固定的20字节

//UDP有两个字段，首部和数据与部分
//UDP首部共八个字节，由四个字段组成，每个字段占2个字节
unsigned char Source_Port[2];//源端口号占UDP报文的2个字节，在需要对方回信时使用，不需要时可全用0。用十六进制表示
unsigned char Destination_Port[2];//目的端口号占UDP报文的2个字节，在终点交付报文时必须使用。用十六进制表示
unsigned short UDP_Length = 0;//UDP用户数据报的长度，最小值是8（仅有首部时），单位是字节
unsigned short CheckSum=0;//检验和，检测UDP用户数据报在传输中是否有错，有错就丢弃。
//UDP数据部分的最大长度为65527字节，因为IP数据报的最大长度为2^16，需要减去8个字节的UDP首部长
...
void Sender_Transport_Layer(unsigned char* Data,unsigned short Data_length)
{
    ...
    //采用内存复制的方式将UDP的各个部分填写
    memcpy(&UDP[0],Source_Port,2);//源端口
    memcpy(&UDP[2],Destination_Port,2);//目的端口
    UDP_Length = 8 + Data_length;//UDP总长度为首部长度8+数据部分长度
    memcpy(&UDP[4],&UDP_Length,2);//UDP长度
    Turn(&UDP[4],2);
    memcpy(&UDP[6],&CheckSum,2);//检验和
    memcpy(&UDP[8],Data,Data_length);//数据部分
    //伪首部填写
    ...
    memcpy(&UDP[6],&CheckSum,2);//将计算出的检验和填入检验和字段
    Turn(&UDP[6],2);
